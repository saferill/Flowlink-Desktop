    ...
    IAdbService adbService) : INetworkService, ISessionManager, ITcpServerProvider, ITcpClientProvider
{
    ...
    // Auto-reconnect: one background task per paired device
    private readonly ConcurrentDictionary<string, CancellationTokenSource> _reconnectTasks = [];
    ...
    /// <summary>
    /// Starts a background reconnect loop for the given device using exponential backoff.
    /// Delays: 1s → 2s → 4s → 8s → 16s → 30s (capped).
    /// Stops automatically when the device connects or when StopAutoReconnect is called.
    /// </summary>
    private void StartAutoReconnect(PairedDevice device)
    {
        // Cancel any existing reconnect task for this device
        StopAutoReconnect(device.Id);

        var cts = new CancellationTokenSource();
        _reconnectTasks[device.Id] = cts;
        ...
        _ = Task.Run(async () =>
        {
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                if (device.IsConnected || cts.Token.IsCancellationRequested)
                    break;

                logger.Info($"[AutoReconnect] Attempting to reconnect to {device.Name}...");
                try
                {
                    await Task.Run(() => ConnectTo(device), cts.Token);
                }
                catch (OperationCanceledException)
                {
                    break;
                ...
                {
                    logger.Warn($"[AutoReconnect] Error reconnecting to {device.Name}: {ex.Message}");
                }
            ...
            _reconnectTasks.TryRemove(device.Id, out _);
            logger.Info($"[AutoReconnect] Stopped for {device.Name}");
        }, cts.Token);
    }

    /// <summary>Cancels the auto-reconnect task for a specific device.</summary>
    private void StopAutoReconnect(string deviceId)
    {
        if (_reconnectTasks.TryRemove(deviceId, out var existingCts))
        {
            existingCts.Cancel();
            existingCts.Dispose();
        }
    ...
    /// <summary>Cancels all active auto-reconnect tasks. Called on app exit.</summary>
    public void StopAllReconnects()
    {
        foreach (var kvp in _reconnectTasks)
        {
            kvp.Value.Cancel();
            kvp.Value.Dispose();
        }
        _reconnectTasks.Clear();
        logger.Info("[AutoReconnect] All reconnect tasks stopped.");
    }
    ...
    private async Task AuthenticatePairedDeviceClient(ServerSession session, PairedDevice pairedDevice, string address)
    {
        ...
        // Stop any active reconnect loop — connection succeeded via server session
        StopAutoReconnect(pairedDevice.Id);
        ...
        try
        {
            ...
            if (pairedDevice is not null)
            {
                ...
                if (pairedDevice.Client is null)
                {
                    ...
                    // Start auto-reconnect if not a forced/manual disconnect
                    if (!forcedDisconnect)
                        StartAutoReconnect(pairedDevice);
                }
            }
            else
            {
    ...
    private async Task AuthenticatePairedDeviceServer(Client client, PairedDevice pairedDevice, string address)
    {
        ...
        // Stop any active reconnect loop — connection succeeded via client
        StopAutoReconnect(pairedDevice.Id);
