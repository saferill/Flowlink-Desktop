...
) : IAdbService
{
    ...
    private const string FlowLinkAndroidPackageId = "com.castle.flowlink";
    ...
    private async Task<AdbDevice> GetFullDeviceInfoAsync(DeviceData deviceData)
    {
        try
        {
            ...
            try
            {
                ...
                // adb shell cat /storage/emulated/0/Android/data/com.castle.flowlink/files/device_info.txt
                // Get the Android ID from the device_info.txt file since we can't directly access the android id of the App 
                await adbClient.ExecuteShellCommandAsync(deviceData, "cat /storage/emulated/0/Android/data/com.castle.flowlink/files/device_info.txt", androidIdReceiver);
    ...
    // executes: adb shell appops set com.castle.flowlink RECEIVE_SENSITIVE_NOTIFICATIONS allow
    private async Task GrantSensitiveNotificationAsync(AdbDevice device)
    {
        ...
        try
        {
            await adbClient.ExecuteShellCommandAsync(device.DeviceData, $"appops set {FlowLinkAndroidPackageId} RECEIVE_SENSITIVE_NOTIFICATIONS allow");
        }
        catch (Exception ex)
        {
