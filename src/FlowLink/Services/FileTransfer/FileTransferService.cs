    ...
    ) : IFileTransferService
{
    ...
    public const int ChunkSize = 524288; // 512KB fallback
    public static readonly IEnumerable<int> PortRange = Enumerable.Range(5152, 18);

    public static int GetAdaptiveChunkSize(long fileSize)
    {
        if (fileSize <= 1024 * 1024) return 64 * 1024;       // 64 KB (Instant for small photos/docs)
        if (fileSize <= 20 * 1024 * 1024) return 256 * 1024;  // 256 KB (Fast for medium files)
        if (fileSize <= 100 * 1024 * 1024) return 1024 * 1024; // 1 MB (High throughput)
        return 4 * 1024 * 1024;                              // 4 MB (Maximum wire-speed for large/4K video)
    }
