...
) : IScreenMirrorService
{
    ...
    public IntPtr CurrentScrcpyHwnd { get; private set; } = IntPtr.Zero;
    public event Action<IntPtr>? OnScrcpyWindowFound;
    public event Action? OnScrcpyWindowClosed;

    // Win32 Interop to find the scrcpy window
    ...
    private static IntPtr FindWindowForProcess(int processId)
    {
        ...
        EnumWindows((hwnd, lParam) =>
        {
            ...
            if (pid == processId)
            {
                ...
                if (className.ToString().Contains("SDL_app"))
                {
                    foundHwnd = hwnd;
    ...
    private void StartProcessMonitoring(Process process, CancellationTokenSource processCts, string deviceSerial)
    {
        ...
        // Start polling for the scrcpy window HWND in the background
        Task.Run(async () =>
        {
            try
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    ...
                    hwnd = FindWindowForProcess(process.Id);
                    if (hwnd != IntPtr.Zero)
                    {
                        CurrentScrcpyHwnd = hwnd;
                        OnScrcpyWindowFound?.Invoke(hwnd);
                        break;
                    }
            ...
            {
                logger.Error("Error finding scrcpy window handle", ex);
            }
        });
        ...
        Task.Run(async () =>
        {
            ...
            finally
            {
                ...
                CurrentScrcpyHwnd = IntPtr.Zero;
                OnScrcpyWindowClosed?.Invoke();
