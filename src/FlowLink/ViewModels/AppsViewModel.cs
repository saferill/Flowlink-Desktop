    ...
    public async Task OpenApp(ApplicationItem app)
    {
        await App.MainWindow.DispatcherQueue.EnqueueAsync(async () =>
        {
            ...
            try
            {
                ...
                var iconPath = IconUtils.GetAppIconFilePath(app.PackageName);
                var started = await ScreenMirrorService.StartScrcpy(DeviceManager.ActiveDevice!, $"--start-app={app.PackageName} --window-title=\"{app.AppName}\"", iconPath);
                if (started)
                {
