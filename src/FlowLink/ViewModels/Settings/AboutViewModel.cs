...
namespace FlowLink.ViewModels.Settings;
public partial class AboutViewModel : ObservableObject
{
    ...
    public string LocalAddressesList
    {
        get
        {
            var addresses = FlowLink.Helpers.NetworkHelper.GetAllValidAddresses();
            if (addresses.Count == 0)
            {
                return "Tidak terhubung ke jaringan";
            }
            return string.Join("\r\n", addresses.Select(a => $"{a.Address}"));
        }
    }
